...
# Use a small/cropped NLCD raster if possible
nlcd_file <- rast("C:/Users/nugen/OneDrive - Western Washington University/App Dev/UrbanizationApp/Annual_NLCD_LndCov_2024_CU_C1V1.tif")


WA_NLCD <- rast("NLCD_90m.tif")
...
get_urban_score <- function(lat, lon, buffer_m = 1000, resolution_factor = 10) { # Main calculation function
  
  nlcd <- rast(nlcd_file) # Load NLCD raster
  ...
  nlcd_ll <- project( # Convert raster to latitude/longitude
    nlcd, # Raster to project
    "EPSG:4326", # WGS84 lat/long coordinate system
    method = "near" # Keeps categorical NLCD values as classes
  )
  
  buffer_deg <- buffer_m / 111000 # Rough conversion from meters to degrees
  
  search_area <- ext( # Create a rough bounding box around the coordinate
    lon - buffer_deg, # Minimum longitude
    lon + buffer_deg, # Maximum longitude
    lat - buffer_deg, # Minimum latitude
    lat + buffer_deg # Maximum latitude
  )
  
  nlcd_crop <- crop(nlcd_ll, search_area) # Crop raster to small rough search area
